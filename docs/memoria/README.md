...
- [incident_popplayer_cloudflare_bloqueio](incident_popplayer_cloudflare_bloqueio.md) — 30/08/2026 — investigação completa de automação do POP Player (popplayer.pro): descartada, site bloqueia qualquer cliente não-navegador (Cloudflare Bot Management), mesmo Chrome real via Playwright/CDP; cadastrado só como app manual (`apps.id_app=32`, sem adapter)
...
- [incident_settings_local_json_malformado](incident_settings_local_json_malformado.md) — `.claude/settings.local.json` com vírgula faltando invalidava o JSON inteiro, fazendo o classificador bloquear merge/push na main mesmo com a regra já cadastrada (26/08/2026)
- [project_central_renovacao_navegador](project_central_renovacao_navegador.md) — ⛔ automação via Playwright/CDP abandonada e revertida em 27/08/2026 (0/4 contra Turnstile interativo); ✅ 30/08/2026 renovação simples testada com sucesso via Claude in Chrome (sem fila, sob demanda — ver Opção B); checkbox interativo pós-bloqueio de sessão continua não testado por esse método
...
- [project_saldo_servidor_automacao](project_saldo_servidor_automacao.md) — 27/08/2026: cron de reconciliação automática do saldo de créditos por servidor (6h) + refresh do status ao vivo no card em /conexoes
