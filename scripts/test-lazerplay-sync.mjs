// Script de teste direto do sync LazerPlay (sem HTTP auth)
// Uso: node scripts/test-lazerplay-sync.mjs
...
const ID_PAINEL = 101;        // painel_servidores.id do LazerPlay
const ID_APP_LAZERPLAY = 2;   // apps.id_app = "Lazer Play"
const API = 'https://api.appacesso.com';
...
async function main() {
  ...
  console.log('📡 Buscando devices do LazerPlay...');
  ...
  for (let i = 0; i < devices.length; i++) {
    ...
    if (existentes.length > 0) {
      ...
      const { rows: ins } = await pool.query(
        ...
        [ID_APP_LAZERPLAY, dev.mac, String(dev.id), validade, dev.model ?? null, ID_PAINEL]
      );
