...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const cliente = await pool.query(`
      SELECT
        c.id_cliente, c.nome, c.observacao, c.score_fidelidade,
        a.id AS id_assinatura, a.plano, a.status,
        a.venc_contrato, a.venc_contas, a.valor,
        s.nome AS servidor
      FROM public.clientes c
      LEFT JOIN public.assinaturas a ON a.id_cliente = c.id_cliente
        AND a.status NOT IN ('cancelado', 'inativo')
      LEFT JOIN public.servidores s ON s.id = a.id_servidor
      WHERE c.id_cliente = (
        SELECT id_cliente FROM public.whatsapp_mensagens
        WHERE telefone = $1 AND id_cliente IS NOT NULL
        LIMIT 1
      )
