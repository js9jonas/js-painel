          ...
          <div className="flex items-center gap-2 mt-3">
            ...
            >
              <span className="w-4 h-4 rounded-full border-2 flex items-center justify-center text-[10px]
                               border-current font-bold">1</span>
              Cliente
            </button>
            <div className="h-px flex-1 bg-zinc-200" />
            <button
              type="button"
              onClick={() => { if (nome.trim()) setStep("assinatura"); }}
              disabled={!nome.trim()}
              className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                step === "assinatura"
                  ? "bg-zinc-900 text-white"
                  : "bg-zinc-100 text-zinc-600 hover:bg-zinc-200 disabled:opacity-40 disabled:cursor-not-allowed"
              }`}
            >
              <span className="w-4 h-4 rounded-full border-2 flex items-center justify-center text-[10px]
                               border-current font-bold">2</span>
              Assinatura
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          ...
          {step === "assinatura" && (
            <div className="space-y-4">
              ...
              {/* Toggle criar assinatura */}
              <label className="flex items-center gap-3 cursor-pointer select-none">
                <div
                  onClick={() => setCriarAssinatura(!criarAssinatura)}
                  className={`w-10 h-6 rounded-full transition-colors flex items-center px-1 ${
                    criarAssinatura ? "bg-zinc-900" : "bg-zinc-300"
                  }`}
                >
                  <div className={`w-4 h-4 rounded-full bg-white shadow transition-transform ${
                    criarAssinatura ? "translate-x-4" : "translate-x-0"
                  }`} />
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className={inputClass}
                      placeholder="Ex: Sala 3, Apto 201..."
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      value={obsAssinatura}
                      onChange={(e) => setObsAssinatura(e.target.value)}
                      rows={2}
                      className="w-full rounded-xl border border-zinc-300 px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-zinc-900 focus:border-transparent transition-all resize-none"
